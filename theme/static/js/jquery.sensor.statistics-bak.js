...
(function($){
    $.fn.extend({
        sensorstatistics: function (settings) {
            var $this = $(this);
            var serviceUrl = 'wfs-fake.php?url=';
            
            function CitisenseSensorWidget(settings){
                // unique id
                var uuid     = new Date().getTime();
                var uri      = serviceUrl + "http://54.154.5.129:8080/lastRecord/webproxy/lastrecord/sensorid/";
                var chart = null;
                ...
                var parseWfsResponse = function(wfsJson) {
                    ...
                    for (var i=0,len=observations.length; i<len;i++) {
                        ...
                        if (observation.Observations.contains.Measurement) {
                            measurements.push(observation.Observations.contains.Measurement);
                        }
                    }
                    return measurements;
                };
					...
					switch (type) {
						...
						case 'historic' : {
                            ...
                            for (var i=0, len=measurements.length; i<len; i++) {
                                ...
                                var value = Number(measurements[i].value);
